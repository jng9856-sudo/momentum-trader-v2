...
interface StockData {
  ...
  obv_trend: string;
  obv_detail: string;
  rs_line_trend: string;
  rs_line_3m_change: number;
  rs_line_divergence: string;
...
const SIG_COLOR: Record<string, { border: string; badge: string; text: string }> = {
    ...
    <div className={`border border-zinc-800 border-l-4 ${sc.border} rounded-xl bg-[#111]`}>
      ...
      <div className="flex items-center justify-between px-4 py-3">
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button onClick={onRemove} className="w-6 h-6 flex items-center justify-center rounded-full bg-zinc-800 hover:bg-red-900 text-zinc-500 hover:text-red-400 transition-colors text-xs">✕</button>
        </div>
      </div>
      ...
      <div className="px-4 pb-4 pt-3">
        ...
        {tab === 'pattern' && (
          <div className="space-y-3">
            {stock.breakout_52w && (
              <div className="p-3 rounded-lg border border-emerald-700 bg-emerald-950/20">
                <p className="text-xs font-bold text-emerald-300 mb-1">🚀 52주 신고가 돌파</p>
                <p className="text-[10px] text-zinc-400" style={{ fontFamily: 'system-ui' }}>{stock.breakout_52w_detail}</p>
              </div>
            )}
            {stock.vcp_is_vcp && (
              <div className="p-3 rounded-lg border border-sky-800 bg-sky-950/20">
                <div className="flex justify-between mb-1">
                  <p className="text-xs font-bold text-sky-300">VCP 패턴</p>
                  <span className="text-xs font-mono text-sky-400">{stock.vcp_score}점</span>
                </div>
                {stock.vcp_pivot && <p className="text-[10px] text-zinc-400">피봇 ${stock.vcp_pivot}</p>}
              </div>
            )}
            {stock.pocket_pivot && (
              <div className="p-3 rounded-lg border border-violet-800 bg-violet-950/20">
                <p className="text-xs font-bold text-violet-300 mb-1">⚡ 포켓 피벗</p>
                <p className="text-[10px] text-zinc-400" style={{ fontFamily: 'system-ui' }}>{stock.pocket_pivot_detail}</p>
              </div>
            )}
            ...
            <div className={`p-3 rounded-lg border ${stock.obv_trend === 'UP' ? 'border-emerald-900 bg-emerald-950/10' : stock.obv_trend === 'DOWN' ? 'border-red-900 bg-red-950/10' : 'border-zinc-800 bg-zinc-900/20'}`}>
              <div className="flex justify-between mb-1">
                ...
                <span className={`text-xs font-bold ${stock.obv_trend === 'UP' ? 'text-emerald-400' : stock.obv_trend === 'DOWN' ? 'text-red-400' : 'text-zinc-400'}`}>
            ...
            {stock.weekly_trend && (
              <div className={`p-3 rounded-lg border ${stock.weekly_is_entry ? 'border-emerald-600 bg-emerald-950/20' : 'border-zinc-800 bg-zinc-900/20'}`}>
                <div className="flex justify-between">
                  ...
                  <span className={`text-xs font-bold ${stock.weekly_trend === 'UPTREND' ? 'text-emerald-400' : stock.weekly_trend === 'DOWNTREND' ? 'text-red-400' : 'text-zinc-400'}`}>
                    {stock.weekly_trend === 'UPTREND' ? '▲ 상승' : stock.weekly_trend === 'DOWNTREND' ? '▼ 하락' : '— 횡보'}
                    {stock.weekly_is_entry ? ' 🎯 최고타점' : ''}
                  </span>
                </div>
              </div>
            )}
          ...
          <div className="space-y-3">
            ...
            {stock.rr_ratio !== null && (
              <div className="p-3 bg-zinc-900/50 rounded-lg border border-zinc-800 flex items-center justify-between">
                <span className="text-[10px] text-zinc-500">R/R 비율</span>
                <span className="text-sm font-bold font-mono text-zinc-200">{stock.rr_label}</span>
              </div>
            )}
            {stock.trail_initial && (
